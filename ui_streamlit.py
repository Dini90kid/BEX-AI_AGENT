...
def page_analyse():
    ...
    if subtask == "Profile CSV(s)":
        ...
        if mode == "Upload CSV file(s)":
            csv_uploads = st.file_uploader("Upload one or more CSV files", type=["csv"], accept_multiple_files=True)

        elif mode == "Upload ZIP of a folder (Streamlit Cloud)":
            zip_upload = st.file_uploader("Upload ZIP of a folder containing CSVs", type=["zip"])

        else:  # Local folder
            folder_path = st.text_input(
                "Local folder path (will scan recursively for *.csv)",
                value="",
                placeholder=r"C:\Users\946558\Downloads\my_csv_folder"
            )

        run = st.button("🔎 Profile", type="primary")
        if run:
            # Resolve inputs -> list[Path] of CSV files
            csv_paths: list[Path] = []

            if csv_uploads:
                tmp_root = Path(tempfile.mkdtemp())
                in_dir = tmp_root / "input"; in_dir.mkdir(parents=True, exist_ok=True)
                for f in csv_uploads:
                    (in_dir / f.name).write_bytes(f.getvalue())
                csv_paths = [p for p in in_dir.iterdir() if p.suffix.lower() == ".csv"]
                if not csv_paths:
                    st.error("No CSV files were saved from uploads."); return

            elif zip_upload:
                root = _extract_zip_to_tmp(zip_upload)
                csv_paths = _iter_files(root, (".csv",))
                if not csv_paths:
                    st.error("No CSV files found inside the ZIP."); return

            else:
                if not folder_path:
                    st.error("Please enter a folder path or choose another mode."); return
                root = Path(folder_path)
                if not root.exists():
                    st.error(f"Path not found: {root}"); return
                csv_paths = _iter_files(root, (".csv",))
                if not csv_paths:
                    st.error("No CSV files found under the given folder."); return

            # Produce profiles for all discovered CSVs
            outputs = {}
            tabs = st.tabs([p.name for p in csv_paths[:10]])  # show up to 10 previews to keep UI snappy
            ...
            for i, p in enumerate(csv_paths):
                ...
                for c in df.columns:
                    ...
                    rpt["columns_profile"].append(info)

                # On-screen for the first few
                if i < len(tabs):
                    with tabs[i]:
                ...
                # Save JSON & Markdown for ZIP
                j = json.dumps(rpt, indent=2).encode()
                m = [f"# Data Profile: {p.name}", f"Rows: {rpt['rows']}", f"Columns: {rpt['columns']}", ""]
                ...
                for c in rpt["columns_profile"]:
                    ...
                    m.append(row)
                outputs[f"{p.name}/{p.stem}_profile.json"] = j
            ...
            st.download_button(
                ...
                data=zip_named_files(outputs),
                file_name="data_profiles.zip",
                mime="application/zip"
            )

    # ---------- keep your existing branches below ----------
    elif subtask == "Reconcile two CSVs (diff by keys)":
        # your reconcile code (unchanged) ...
        ...
    else:
        # your BW Dependency code (unchanged) ...
        ...
